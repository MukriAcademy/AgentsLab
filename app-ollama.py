...
if not USE_AGENT:
    ...
    for model in ollama_models:
        try:
            ...
            print(f"⚠️ Ollama {model} failed: {str(e)[:80]}")
            continue
...
def ask_agent(question: str, chat_history: List) -> Tuple[List, List]:
    ...
    try:
        if agent_executor:
            ...
            # Manual tool routing with keyword matching
            print(f"\n🤖 Processing: {question}")
            
            # Check for document-related keywords first
            doc_keywords = ["document", "file", "upload", "pdf", "text", "holiday", "list", "content", 
                          "what is", "summarize", "explain", "show", "tell me about", "information",
                          "contains", "says", "written", "mentioned"]
            calc_keywords = ["calculate", "compute", "math", "sum", "multiply", "divide", "add", "subtract"]
            
            question_lower = question.lower()
            
            # Direct keyword matching for better accuracy
            if any(keyword in question_lower for keyword in doc_keywords):
                print(f"🔧 Using document_qa_tool (keyword match)")
                tool_result = document_qa_tool.invoke(question)
                
                # If document found info, let LLM summarize it properly
                if "No documents" in tool_result or "No relevant" in tool_result:
                    answer = f"📄 {tool_result}"
                else:
                    # Let LLM provide a natural answer based on retrieved context
                    summarization_prompt = f"""Based on the following information from the uploaded documents:

{tool_result}

User's question: {question}

Please provide a clear, direct, and helpful answer to the user's question based on the information above. Be specific and cite the information from the document."""
                    
                    llm_answer = llm.invoke(summarization_prompt).content
                    answer = f"📄 {llm_answer}"
                    
            elif any(keyword in question_lower for keyword in calc_keywords) or any(char in question for char in "+-*/="):
                print(f"🔧 Using calculator_tool (keyword match)")
                import re
                ...
                # For general questions, use web search
                print(f"🔧 Using web_search_tool (default)")
        ...
        try:
            ...
            # Very broad document keywords for fallback
            if any(word in question_lower for word in ["document", "file", "upload", "pdf", "text", "holiday", 
                                                        "list", "show", "what", "tell", "explain", "content",
                                                        "information", "data", "contains", "says"]):
                print("🔄 Fallback: Using document_qa_tool")
                tool_result = document_qa_tool.invoke(question)
                
                if "No documents" not in tool_result and "No relevant" not in tool_result:
                    # Try to get LLM summary
                    try:
                        summary = llm.invoke(f"Summarize this for the user:\n{tool_result}\n\nQuestion: {question}").content
                        answer = f"📄 {summary}"
                    except:
                        answer = f"📄 {tool_result}"
                else:
            ...
            elif any(char in question for char in "+-*/") or any(word in question_lower for word in ["calculate", "compute", "math"]):
                print("🔄 Fallback: Using calculator_tool")
                import re
                expr = re.search(r'[\d\+\-\*\/\(\)\s\.]+', question)
                if expr:
                    answer = f"🧮 Result: {calculator_tool.invoke(expr.group().strip())}"
                else:
                    answer = "Could not extract a math expression."
            else:
                print("🔄 Fallback: Using web_search_tool")
                answer = f"🌐 {web_search_tool.invoke(question)}"
...
if __name__ == "__main__":
    demo.launch(theme=gr.themes.Soft(),server_name="127.0.0.1", server_port=7878, share=True)
